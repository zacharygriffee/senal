...
import {act, currentTada} from "./act.js";
...
function senal(object = {}, config = {}) {
    ...
    function subscribe(target, property) {
        if (!isIgnored && (isPropertyObservable(target, property) || isArrayOrArrayLikeAndArrayToObjectIsTrue(target[property]))) {
            if (target[property] && !Object.getOwnPropertyDescriptor(target, property)?.writable) {
