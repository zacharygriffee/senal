 ...
 * @param {boolean} [config.ignoreNestedFunctions=false] If true, and config.deep=true will not make nested functions
 * reactive.
...
function senal(object = {}, config = {}) {
    ...
    try {
        ...
        // An non-extensible object will fail
        return object;
    }

    if (deep && !isIgnored) {
        for (const [key, value] of Object.entries(object)) {
            if (ignoreNestedFunctions && isFunction(value)) continue;
            if (isValueObservable(value) || isArrayOrArrayLikeAndArrayToObjectIsTrue(value)) {
                if (!Array.isArray(value) || isArrayOrArrayLikeAndArrayToObjectIsTrue(value)) {
                    try {
                        ...
                        // Objects that have a getter but not a setter will be bypassed
                        if (!!descript.set || descript.writable === true) {
                            object[key] = senal(value);
                        }
                    } catch (e) {
                        // Any errors here is likely object cannot be made reactive.
                        // Happens more often in browser.
                    }
                }
            }
        }
                ...
                for (const tadaFunction of subscriptions[property]) {
                    if (tadaFunction) {
                        if (tadaFunction.completed || tadaFunction.errored) {
                            ...
                            act(_inciter(proxy, reason, meta), tadaFunction);
                        }
                    }
                }
    ...
    const proxy = new Proxy(object, {
        ...(object[invokable] ? {
            apply(target, thisArg, args) {
                ...
                if (currentTada?.readOnly) {
                    // We just need the property name in readonly mode.
                    advertise(target, __lastInvokeProperty, "invoke", "invocation", {thisArg, args});
                } else {
        ...
        set(target, property, value, receiver) {
            ...
            if (deep && (isValueObservable(value) || isArrayOrArrayLikeAndArrayToObjectIsTrue(value)) && !(ignoreNestedFunctions && isFunction(value))) {
                if (!Array.isArray(value) || isArrayOrArrayLikeAndArrayToObjectIsTrue(value))
                    value = senal(value);
            }
            if (subscribeOnSet) {
