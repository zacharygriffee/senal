 ...
 * @param {boolean} [config.subscribeOnSet=false] Whether a set inside a computed function will trigger subscription.
 * By default, you have to access a property in a computed for the property changes to be subscribed to.  IF set to true
 * setting a property inside a computed will trigger its subscription if it isn't already, `e.g. object.property = 42;`
 * will cause a subscription to computed. If not careful, you could trigger stack overflow.
 ...
 * @returns {Proxy} A proxy of the object. Operations on the proxy will signal to computed function.
...
function senals(object = {}, config = {}) {
    ...
    if (!isValueObservable(object)/*object == null || (typeof object !== "object" && typeof object !== "function")*/) {
        throw new Error("Observe first argument must be an object");
    }
    ...
    function notify(property, meta) {
        if (!isIgnored && subscriptions[property]) {
            ...
            if (!queued[property]) {
                ...
                for (const computedFunction of subscriptions[property]) {
                    if (computedFunction) {
                        if (computedFunction.completed || computedFunction.errored) {
                            delete subscriptions[property];
                        } else {
                            act(computedFunction, _inciter(proxy, "property", meta));
                        }
                    }
                }
    ...
    const proxy = new Proxy(object, {
        get(target, property, receiver) {
            ...
            // if (property === reactiveSymbol) return configBit;
