...
function tada(observer, ...filters) {
    if (!isObserverOrNext(observer)) {
        observer = (_ => null);
        // throw new Error("Tada function should be an observer or function.");
    }
    ...
    function subscribe(observerOrNext) {
        ...
        if (isObserver(observerOrNext)) {
            ...
            for (const type of ["error", "complete"]) {
                _observerOrNext[type] = (observerOrNext[type] ?? (_ => null));
            }
    ...
    function unsubscribe() {
        observer.complete();
        for (const k of Object.keys(_subscriptions)) {
