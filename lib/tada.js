...
function tada(observer, ...filters) {
    ...
    if (!(observer.inQueue && !observer.errored && !observer.completed) && passFilter({filters}, {reason: "initial"})) {
        act(observer, _inciter(observer, "initial"));
        observer.started = true;
    }

    return new Proxy(executable, {
        get(target, property, receiver) {
            if (observer[property]) return Reflect.get(observer, property, receiver);
        },
        set(target, property, value, receiver) {
            if (observerCompletedOrErrored(observer)) return true;
            if (observer[property]) {
                return Reflect.set(observer, property, value, receiver);
            }
        }
    });
    ...
    function addFilter(...filters) {
        if (observerCompletedOrErrored(observer)) return observer;
