...
import {subscribe, reactive, tadaSymbol, notify} from "./symbols.js";
import {_inciter, inciter} from "./inciter.js";
import {collect, collectLastTick, collectNextTick, collectNextTickLive} from "./collector.js";
...
function tada(observer, ...filters) {
    ...
    if (!observer?.[tadaSymbol]) {
        ...
        observer.filters = filters.length ? filters.flat(Infinity) : ["initial", "property", "collection", "manual"];
    ...
    function retro(subscribeOnSet = false) {
        ...
        for (const {cause, property, type} of result) {
            if (subscribeOnSet || type === "get") {
                if (cause[reactive]) {
                    cause[subscribe](observer, property);
    ...
    function pronto(subscribeOnSet = false) {
        ...
        collectNextTickLive(({cause, property, type}) => {
            if (subscribeOnSet || type === "get") {
                if (cause[reactive]) {
                    cause[subscribe](observer, property);
                    cause[notify](property);
